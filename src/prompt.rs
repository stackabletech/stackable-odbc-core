...
//!
//! Core deliberately ships no implementation. Every one it could offer — open a
//! browser, put up a dialog — needs a dependency and a platform, and neither
//! belongs in the database-independent half of a driver.

use crate::errors::OdbcError;

/// Presents a URL to the user while a connection is being established.
///
/// Implemented by a driver that needs interactive authentication — an OAuth 2.0
/// external-authentication flow, for instance, where the data source answers
/// the initial request with a login URL a human has to visit. Obtained from
...
pub trait Prompter: Send + Sync {
    ...
    /// # Errors
    ///
    /// Whatever the implementation could not do — no display, no browser, no
