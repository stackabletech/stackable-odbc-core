...
pub trait Backend: Sized + Send + Sync + 'static {
    ...
    /// connect, so `3D000` arrives from a function whose own diagnostics table
    /// does not list it. Degrading it would tell the application its connection
    ...
    /// and it warns applications off using it as a truncation mechanism at
    /// all: "this mechanism should not be used by applications to truncate
    /// data; to truncate data received, an application should specify the
    /// maximum buffer length in the *BufferLength* argument in `SQLBindCol` or
    ...
    /// the answer to decide whether the driver is ODBC 3.x, so a wrong or
    /// missing one costs the connection its 3.x features.
    ///
    /// **That group needs no override.** `NotImplemented` falls through to
    /// [`default_get_info`], which answers `SQL_DRIVER_NAME` and
    /// `SQL_DRIVER_VER` from [`Backend::driver_name`] and
    /// [`Backend::driver_version`], and `SQL_DRIVER_ODBC_VER`,
    /// `SQL_ASYNC_DBC_FUNCTIONS` and `SQL_MAX_CONCURRENT_ACTIVITIES` from what
    /// core knows about itself. Those are what the Driver Manager needs, and
    /// the two hooks behind them are required declarations rather than a
    /// checklist item.
    ///
    ...
    /// [`Backend::supports_catalogs`] already returned `true`, since a backend
    /// that says it has no catalogs gets an empty result set without being
    /// asked. So a backend that claims catalogs and leaves this defaulted is a
    /// driver bug, which core reports as `HYC00` rather than silently returning
    /// nothing.
    ///
    ...
    ///
    /// - **Standalone.** The token carries its own channel and shares nothing
    ///   with the connection. libpq's `PGcancel` (a snapshot of the backend
    ///   PID and secret key, used over a fresh socket) and MySQL's second
    ///   connection issuing `KILL QUERY` are both this shape. Prefer it.
    /// - **Aliasing.** The token holds the connection itself, sound only where
    ///   the backend's own client library documents concurrent use.
    ...
    /// close before `sqlite3_interrupt()` returns". The same reasoning applies
    /// to any aliasing token, not only SQLite's.
    ///
    ...
    /// unused, which is harmless, since it is cheap and the statement may yet
    /// make a real call later.
    ///
    ...
    /// Reusing one token across executions would leave a cancelled statement
    /// permanently unusable, since `Backend::cancel` marks the token and the
    /// next execution would find it marked. The spec requires the opposite:
    ...
    /// at all."
    ///
    /// Build the token with the connection's parameters in hand here; do not
    /// defer the real assembly to `cancel`. MariaDB's ODBC-401 is the failure
    /// this rule exists to prevent: its cancel channel was assembled lazily, by
    /// which time the original connection's TLS settings were out of reach, so
    /// cancelling an encrypted connection failed silently.
    ///
    /// A backend whose cancellation needs a value not known until execution
    /// (e.g. a query id) returns an empty shared slot here, such as an
    /// `Arc<Mutex<Option<QueryId>>>`, and fills it from whichever
    /// statement-producing method actually runs the work. A catalog method like
    /// `tables` is a real query with its own id on some backends, not only
    /// `exec_direct`/`prepare`/`execute`, and every one of them receives the
    /// same token.
    ...
    /// and that is what keeps this call off the *guarded state*: there is no
    /// `&mut Self::Statement` or `&Self::Connection` reachable through it, so a
    /// backend cannot reach into a handle another thread may be mutating.
    ///
    /// That is **not** the same as "core never holds the connection's lock
    /// while calling this". Whether it does depends on which of `SQLCancel`'s
    /// two cases applies:
    ///
    ...
    /// itself waits for this connection's lock, such as a call back into
    /// another `SQLxxx` entry point on the same connection, or the idle-path
    /// case deadlocks the calling thread. Blocking on something
    ...
    /// core discards the backend's SQLSTATE and reports `HY008` instead, per
    ...
    /// `SQL_CURSOR_COMMIT_BEHAVIOR` outright: from the backend's own
    ...
    ///
    /// Takes no connection, unlike the capability methods below.
    /// `SQLProcedures`, `SQLProcedureColumns`, `SQLColumnPrivileges` and
    /// `SQLTablePrivileges` return an empty result set without resolving one,
    /// and requiring a connection here would put a connection lookup, and an
    /// error path, into paths that have neither. A driver whose identifier
    /// limit varies by server version reports the widest form here and narrows
    /// the `SQL_MAX_*_NAME_LEN` answers through the per-connection hooks.
    ...
    /// same breath.
    ///
    /// **Required** rather than defaulted: a defaulted `true` would silently
    /// reproduce that contradiction in the next catalog-less backend, and the
    /// fact is one every backend author already knows.
    ///
    ...
    /// answers the whole pre-connect identity group itself, so a driver never
    /// has to override [`Backend::get_info_pre_connect`] for the Driver
    /// Manager's benefit.
    ...
    ///
    /// Required because core cannot know it, and hard-coding it would make core
    /// contradict itself: claiming `SQL_SC_SQL92_ENTRY` while separately
    ...
    /// no escape character, which is the spec's answer for that case, and one
    /// core cannot distinguish from a backend that simply never set it.
    ///
    ...
    /// `txn_isolation_options` must override this, or the default reports
    /// `NotImplemented` rather than accepting a level it would then silently
    /// fail to apply.
    ///
...
pub trait StatementBackend: Send + Sync {
    ...
    /// use one type for both traits, since nothing here requires them to differ.
    ///
    /// This exists so the fetch path keeps its causal chain. `fetch` and
    /// `get_data` are the hottest error path in the crate, and returning
    /// `OdbcError` directly would make a driver flatten its own error into a
    /// string at every call, which is what `Backend::Error` exists to stop
    /// everywhere else.
    type Error: Into<OdbcError> + From<OdbcError> + std::error::Error + Send + Sync + 'static;
    ...
    /// backend must not also report a loss core is going to detect, which
    ...
    ///
    /// Returns `Option` rather than a collection because `get_data` is the
    /// hottest path in the crate, and a `Vec` would allocate on every column of
    /// every row to carry nothing in the overwhelming case.
    ...
    /// a `SQLSMALLINT *`. `u16` would let a backend name a count the driver
    /// cannot report, and push a fallible narrowing into core for a value the
    /// backend already knew was out of range.
    ...
    /// metadata of a wide result set would otherwise pay for a clone each
    /// time.
    ///
    ...
    /// function rather than hand-building an `OdbcError` here. Core reports
    /// what this method returns and does not rewrite it into `07009` "column
    /// number out of range".
    fn describe_col(
        &self,
        _col: u16,
    ...
    ///   here is reported and the discard happens anyway, so the application is
    ///   not left holding a cursor it cannot clear.
...
///
/// Backends should call this at the end of their `get_info` match, before the
/// `_ =>` arm, so they do not duplicate the arms it already answers. Returns
/// `None` for anything driver-specific.
///
...
pub fn default_get_info<B: Backend>(
    ...
    match info_type {
        ...
        // would let a backend quote identifiers one way and tell the
        // application another. The spec's "if the data source does not
        // support quoted identifiers, a blank is returned" is the
        // empty-dialect case.
        ...
        // Not `widths.identifier_len`, because a cursor name is an
        ...
        // spec, not SQLUSMALLINT, as the info-type conformance test finds
        ...
        // streams rows from the backend as the application asks for them, so
        // it makes no such promise about rows it has not read yet. The spec puts
        ...
        // is unknown", which is correct as a shared default, unlike the two
        // above.
        ...
        // Core cannot know the conformance level, and hard-coding it would
        ...
        // `SQL_GD_BLOCK` is absent on purpose: it describes reading a row
        ...
        // the one value that arm accepts unchanged, since every other value is
...
pub fn common_get_info_raw<B: Backend>(
    ...
    match info_type {
        ...
        // reserves. That subtraction is what the spec defines for this info
        // type, applied once here rather than in each backend. The list is a
...
mod tests {
    ...
    /// `SQL_ALTER_TABLE` and `SQL_OJ_CAPABILITIES` are capability bitmaps,
    /// where a defaulted `0` claims "this data source cannot do this at all".
    /// Both come from required `Backend` methods, so a backend author states
    /// the fact rather than inheriting a silent understatement.
    #[test]
    ...
    /// An info type the spec declares as a `"Y"`/`"N"` string needs an arm of
    /// its own: the shape-aware fallback gives it `""`, which is the right
    /// *shape* but not a value in its value list.
    ...
    ///
    /// Anything else belongs on a `Backend` method instead, meaning any value
    /// that is a falsifiable statement about the *data source*. See AGENTS.md,
    ...
    /// Hard-coding a claim about the data source into `default_get_info` fails
    ...
    fn default_get_info_answers_are_backend_derived_or_declared_core_facts() {
        ...
        for info_type in crate::conformance::all_info_types() {
            ...
            match (mine.is_some() && mine == theirs, declared) {
                ...
                // fact, so the entry outlived the value it described.
    ...
    /// exists. The list exists so it can only shrink: a new hard-coded
    ...
    ///
    /// That scan is 131 072 evaluations, which costs nothing on stable but is
    /// a large share of the Miri job, since Miri interprets every one of them.
    /// `common_get_info_raw` contains no `unsafe`, so Miri has nothing to check
    /// here and would pay purely to re-run a classification stable already
    /// made. Same reasoning as
    ...
    fn common_get_info_raw_answers_are_backend_derived_or_declared() {
        ...
        for raw in 0..=u16::MAX {
            ...
            match (mine.is_some() && mine == theirs, declared) {
                ...
                // Backend-derived (or unanswered) but still listed, so the
                // entry outlived the value it described.
    ...
    /// catalog result sets and 128 here, which would be two different answers
    /// about one limit.
    #[test]
