...
/// force. That is the exact defect this whole mechanism removes, so a backend
...
pub enum QueryTimeout {
    /// The data source itself enforces the deadline — a server-side statement
    /// timeout, a session variable, a per-query option.
    ///
    ...
    /// cancelled call fails, and it is what turns the backend's own symptom —
    /// a closed socket, an aborted query — into the `HYT00` the application is
    /// waiting to see.
    ///
    /// [`Backend::cancel`]: crate::backend::Backend::cancel
